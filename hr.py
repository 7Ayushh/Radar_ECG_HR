...
def process_ecg_file(filename):
    ...
    # Thresholding sampling frequency distribution
    init = 1
    ...
    # Window selection for stable sampling frequency
    segment_length = 2000
    ...
    # Sub-window selection for least perturbed segment
    err_record = []
    ...
    for i in range(iter):
        ...
        lamb = 2
        amp = 200
        max_val = amp+escape  # Changed from 'max' to avoid built-in conflict
        min_val = amp-lamb*escape  # Changed from 'min' to avoid built-in conflict
        fs = min_val + n*(max_val-min_val)
...
for file in csv_files:
    ...
    print(f"  Mean BPM: {np.mean(bpm):.2f}")
    print(f"  Mean FS: {np.mean(fs):.2f}")